...
<html lang="es">
  ...
  <body style="margin:0;padding:0;background:#0D0D0D;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:linear-gradient(135deg, #0D0D0D 0%, #021e3a 50%, #0D0D0D 100%);padding:32px 0;">
      <tr>
        <td align="center">
          <table role="presentation" width="640" cellspacing="0" cellpadding="0" style="max-width:640px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(4,77,140,0.25);">
            ${content}
          </table>
        </td>
      </tr>
...
export function buildClientEmailHtml(clientName: string): string {
    ...
    const logoUrl = 'https://consultoria.dccomunicacionessas.com/img/logo-horizontal.png';
    ...
    return baseLayout(
      ...
      <tr>
        <td style="background:#044D8C;padding:32px 24px;text-align:center;">
          <h1 style="margin:0;color:#ffffff;font-size:28px;line-height:1.2;">✅ Formulario Recibido</h1>
        </td>
      </tr>
      <tr>
        <td style="padding:28px 24px;color:#1f2937;font-size:16px;line-height:1.7;">
          ...
          <div style="background:rgba(242,210,48,0.08);border-left:4px solid #F2D230;border-radius:8px;padding:16px;">
            <p style="margin:0 0 10px 0;color:#1f2937;"><strong>💡 Próximos pasos:</strong></p>
      ...
      <tr>
        <td style="background:#f9fafb;padding:20px 24px;text-align:center;border-top:1px solid #e5e7eb;color:#6b7280;font-size:13px;line-height:1.5;">
          ...
          <p style="margin:0;">© ${year} D&C IDEM COMUNICACIONES S.A.S. Todos los derechos reservados.</p>
        </td>
      </tr>
    `,
    );
...
export function buildAdminEmailHtml(clientName: string, clientEmail: string, submittedAt: string): string {
    ...
    return baseLayout(
      ...
      <tr>
        <td style="padding:28px 24px;color:#1f2937;font-size:16px;line-height:1.7;">
          <p style="margin:0 0 16px 0;font-weight:600;color:#0D0D0D;">Se ha recibido un nuevo formulario de requerimientos de software.</p>
          <div style="background:rgba(4,77,140,0.05);border:2px solid #044D8C;border-radius:10px;padding:16px;margin-bottom:18px;">
            <p style="margin:0 0 12px 0;color:#044D8C;font-weight:700;">Información del Cliente</p>
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
              <tr>
                <td style="padding:6px 10px 6px 0;color:#6b7280;font-weight:600;width:130px;">👤 Cliente:</td>
                <td style="padding:6px 0;color:#0D0D0D;">${safeClientName}</td>
              </tr>
              <tr>
                <td style="padding:6px 10px 6px 0;color:#6b7280;font-weight:600;">📧 Email:</td>
                <td style="padding:6px 0;color:#0D0D0D;">${safeClientEmail}</td>
              </tr>
              <tr>
                <td style="padding:6px 10px 6px 0;color:#6b7280;font-weight:600;">📅 Fecha:</td>
                <td style="padding:6px 0;color:#0D0D0D;">${safeSubmittedAt}</td>
              </tr>
            </table>
          </div>
          <div style="background:rgba(242,210,48,0.08);border-left:4px solid #F2D230;border-radius:8px;padding:14px;margin-bottom:16px;">
            <p style="margin:0 0 8px 0;color:#1f2937;"><strong>⚡ Acción requerida:</strong></p>
            <p style="margin:0 0 4px 0;color:#374151;">• Revisa el PDF adjunto con todos los detalles</p>
            <p style="margin:0 0 4px 0;color:#374151;">• Contacta al cliente en las próximas 24-48 horas</p>
            <p style="margin:0;color:#374151;">• Agenda una reunión inicial para discutir el proyecto</p>
          </div>
      ...
      <tr>
        <td style="background:#f9fafb;padding:20px 24px;text-align:center;border-top:1px solid #e5e7eb;color:#6b7280;font-size:13px;line-height:1.5;">
          ...
          <p style="margin:0;">© ${year} D&C IDEM COMUNICACIONES S.A.S.</p>
        </td>
      </tr>
    `,
    );
